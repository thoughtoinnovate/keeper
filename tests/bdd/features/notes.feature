Feature: Notes and tasks
  ...
  Scenario: Capture and retrieve a task
    ...
    And I add a note "Fix auth bug" in bucket "@default/work" with priority "!p1" and due date "^2026-02-01"
    Then the output should contain "[✓] Saved"
    When I get notes for bucket "@default/work"
  ...
  Scenario: Duplicate detection
    Given a fresh keeper home
    When I start the daemon with password "pass123"
    And I add a note "Duplicate test" in bucket "@default/inbox" with priority "!p2" and due date "^2026-02-02"
    And I add a note "Duplicate test" in bucket "@default/inbox" with priority "!p2" and due date "^2026-02-02"
  ...
  Scenario: Invalid due date is rejected
    Given a fresh keeper home
    When I start the daemon with password "pass123"
    And I add a note "Bad date" in bucket "@default/work" with priority "!p1" and due date "^2026-02-99"
